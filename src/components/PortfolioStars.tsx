...
export default function PortfolioStars() {
  ...
  useEffect(() => {
    const fetchStars = async () => {
      try {
        // Try our API first
        const response = await fetch('/api/github-stars?owner=ankitgpt18&repo=sleek-portfolio')
        ...
        if (data.success && data.stars > 0) {
          ...
          const githubResponse = await fetch('https://api.github.com/repos/ankitgpt18/sleek-portfolio')
